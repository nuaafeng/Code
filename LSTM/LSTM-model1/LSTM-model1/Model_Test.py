...
def select_feat(train_data, valid_data, test_data, select_all=True):
    y_train, y_valid,y_true = train_data[:,26], valid_data[:,26],test_data[:,26]#-1表示倒数第一列
    ...
    return raw_x_train[:,feat_idx], raw_x_valid[:,feat_idx], raw_x_test[:,feat_idx], y_train, y_valid,y_true
...
#DATASET生成
x_train, x_valid, x_test, y_train, y_valid,y_true = select_feat(train_data, valid_data, test_data, config['select_all'])
...
#测试
import csv
def save_pred(preds, file,y_true):
    ...
    with open(file, 'w') as fp:
        ...
        for i, p in enumerate(preds):#enumerate 枚举对象可以用for循环遍历
            writer.writerow([i, p])
            writer2.add_scalar("Pred-weight",preds[[i],],i)
            writer3.add_scalars("Pred-weight/all",{'true':y_true[[i],],'pred':preds[[i],]},i)
...
save_pred(preds, 'pred-weight.csv',y_true)